...
void draw(FrameBuffer& frame)
{
	...
	SDL_UpdateTexture(texture, NULL, pixels.data(), GAMEBOY_WIDTH * sizeof(uint32_t));
...
int main(int argc, char* args[])
{
	...
	bool runningCpu = false;
	SDL_Window* window = SDL_CreateWindow("SDL2 ImGui Renderer", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 800, 600, SDL_WINDOW_OPENGL);
	...
	texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_TARGET, screenWidth, screenHeight);
	...
	while (run)
	{
		...
		if (!runningCpu) {
			...
			if (ImGui::Button("Run Until 0x006A")) {
				while (registers.pc != 0x6A) {
					cpu->step();
					registers = cpu->getRegisters();
				}
			}
			...
			ImGui::Text("GPU Registers: ");
			ImGui::Text("LY: 0x%X; SY: 0x%X; XY: 0x%X", cpu->gpu.line, cpu->gpu.scrollY, cpu->gpu.scrollX);
			/*
			ImGui::Spacing();
			ImGui::Text("Stack: ");
